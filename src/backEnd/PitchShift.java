 ...
 * Edited for M,Sc. thesis by @author mirac
 */
public class PitchShift {
        ...
        public PitchShift(MakamBox obj) {
                this(obj.getWavefile());
        }
...
//        	}
         ...
         */
        public void setPitchShift(float holderComma) {
        	pitchShift = (float) Math.pow(2, (holderComma/53));
        ...
        public void shiftPitch(){
        	running = true;
        	shiftedOut = new float[this.MAX_FRAME_LENGTH];
        	smbPitchShift(input,shiftedOut,0,input.length);
        }
        public void shiftPitch(float[] in){
        	running = true;
        	shiftedOut = new float[in.length];
        	smbPitchShift(in,shiftedOut,0,in.length);
        }
        public Wavefile getShiftedWave() throws Exception{
        	shiftedWave = new Wavefile(format,shiftedOut,"shifted.wav");
        	return shiftedWave;
         ...
         */
        public synchronized void smbPitchShift(final float[] indata, final float[] outdata, final int offset, final int numSampsToProcess) {
        	shiftThread = new Thread(new Runnable(){
        		@Override
				public void run(){
        			 int gRover = 0;
                     ...
                     expct = 2.*M_PI*stepSize/fftFrameSize;
                     inFifoLatency = fftFrameSize-stepSize;
                     if (gRover == 0) gRover = inFifoLatency;

                     MakamBoxAnalysis.progressBar.setMaximum(numSampsToProcess-2);
                     /* main processing loop */
                     for (i = 0; i < numSampsToProcess && running==true; i++){
                             ...
                             if (gRover >= fftFrameSize) {
                                     ...
                                     for (k = 0; k < fftFrameSize;k++) {
                                             window = -.5*Math.cos(2.*M_PI*k/fftFrameSize)+.5;
                             ...
                             MakamBoxAnalysis.progressBar.setValue(i);
                     }
        		}
        	});
        	shiftThread.start();
